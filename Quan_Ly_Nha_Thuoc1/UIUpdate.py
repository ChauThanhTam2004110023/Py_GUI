...
class UIUpdate():
    def __init__(self, username , root):
        ...
        self.lbfframe = LabelFrame(self.frame, text='Tìm Kiếm Thuốc', font='arail 15 bold', width=900, height=600)
        ...
        self.entry_1 = Entry(self.frame, bd=3, width=20, font='arail 10')
        self.entry_1.place(x=300, y=70)

        lb_2 = Label(self.frame, text='Tên thuốc', font='arail 10 bold', fg='blue')
        lb_2.place(x=100, y=100)
        self.entry_2 = Entry(self.frame, bd=3, width=20, font='arail 10')
        self.entry_2.place(x=300, y=100)

        lb_3 = Label(self.frame, text='Số lượng', font='arail 10 bold', fg='blue')
        lb_3.place(x=100, y=130)
        self.entry_3 = Entry(self.frame, bd=3, width=20, font='arail 10')
        self.entry_3.place(x=300, y=130)

        button_add = Button(self.frame, text='Thêm', width=10, font='arail 10 bold', command=self.add_data)
        button_add.place(x=400, y=580)

        button_remove = Button(self.frame, text='Xóa', width=10, font='arail 10 bold', command=self.remove_data)
        button_remove.place(x=500, y=580)

        button_edit = Button(self.frame, text='Sửa', width=10, font='arail 10 bold', command=self.edit_data)
        button_edit.place(x=600, y=580)

        lb_4 = Label(self.frame, text='Giá thuốc', font='arail 10 bold', fg='blue')
        lb_4.place(x=100, y=160)
        self.entry_4 = Entry(self.frame, bd=3, width=20, font='arail 10')
        self.entry_4.place(x=300, y=160)

        lb_5 = Label(self.frame, text='Hàm lượng', font='arail 10 bold', fg='blue')
        lb_5.place(x=100, y=190)
        self.entry_5 = Entry(self.frame, bd=3, width=20, font='arail 10')
        self.entry_5.place(x=300, y=190)

        lb_6 = Label(self.frame, text='Nhà cung cấp', font='arail 10 bold', fg='blue')
        lb_6.place(x=100, y=220)
        self.combobox_6 = ttk.Combobox(self.frame,  values=['Click', 'To', 'Load'], width=10)
        self.combobox_6.place(x=300, y=220)
        self.combobox_6.bind("<<ComboboxSelected>>", self.loadComboboxValue)


        lb_7 = Label(self.frame, text='Đơn vị', font='arail 10 bold', fg='blue')
        lb_7.place(x=100, y=250)
        self.entry_7 = Entry(self.frame, bd=3, width=20, font='arail 10')
        self.entry_7.place(x=300, y=250)

        lb_8 = Label(self.frame, text='HSD', font='arail 10 bold', fg='blue')
        lb_8.place(x=100, y=280)
        self.entry_8 = Entry(self.frame, bd=3, width=20, font='arail 10')
        self.entry_8.place(x=300, y=280)

        button = Button(self.frame, text='Tìm kiếm', width=10, font='arail 10 bold', command=self.search_data)
        ...
        lb_9 = Label(self.frame, text='Tên loại thuốc', font='arail 10 bold', fg='blue')
    ...
    # đọc dữ liệu từ file Excel và tìm kiếm dữ liệu
        ...
        filtered_df = df[df["name_thuoc"].str.contains(search_value, case=False)]
    ...
    def add_data(self):
        ...
        new_row = [max_id + 1, name_thuoc, so_luong, gia_thuoc, ham_luong, nha_cung_cap, don_vi, hsd]
    ...
    def remove_data(self):
        ...
        if not select_now:
            messagebox.showerror("Hãy chọn 1 hàng để xóa")
            return
